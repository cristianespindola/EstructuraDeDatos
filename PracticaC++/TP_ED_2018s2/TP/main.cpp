...
void agregarChildren(string vocab[], int vocabSize, GenTree t) {
    for(int i = 0; i < vocabSize; i++) {
        GenTree c = leaf(value(t) + vocab[i]);
        addChild(t, c);
    }
}
...
void generate(string vocab[], int vocabSize, int maxLevel, int level, GenTree& t) {
  /// COMPLETAR
  if(maxLevel > level) {
    agregarChildren(vocab, vocabSize, t);
    for(int i = 0; i < length(children(t)); i++) {
        GenTree c = getAt(children(t), i);
        generate(vocab, vocabSize, maxLevel, level + 1, c);
    }
  }
}
