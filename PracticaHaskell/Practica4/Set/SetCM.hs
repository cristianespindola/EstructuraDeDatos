module SetCM(Set, emptyS, addS, belongs, sizeS, removeS, unionS, setToList, maximoC) where
...
removeS e (MKS xs ys n) = MKS (borrar e xs) (removerM e ys) (n+1)
...
removerM :: Eq a => a -> [a] -> [a]
removerM x [] = []
removerM x (y:ys) = if x == y
						then ys
						else y : (removerM x ys)
